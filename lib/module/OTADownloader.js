...
async function checkNetworkConnectivity(url, headers) {
...
async function performSingleDownload(url, destPath, onProgress, headers) {
  const {
    promise
  } = RNFS.downloadFile({
    fromUrl: url,
...
async function downloadWithRetry(url, destPath, onProgress, headers) {
...
class OTADownloaderClass {
  ...
  async downloadBundle(url, onProgress, headers) {
