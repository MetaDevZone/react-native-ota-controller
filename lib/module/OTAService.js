...
import { OTA_API_BASE_URL, OTA_BUNDLE_FILE_NAME, OTA_BUNDLE_FILE_NAME_IOS, OTA_CHECK_UPDATE_PATH, OTA_EVENTS_PATH } from "./OTAConstants.js";
...
class OTAServiceClass {
   ...
   */
  async reportEvent(event, metadata) {
    const apiKey = this.config?.apiKey;
    if (!apiKey) {
      console.warn('OTA: Cannot report event — apiKey is not configured.');
      return false;
    }
    const platform = this.config?.platform ?? (Platform.OS === 'ios' ? 'ios' : 'android');
  ...
  async checkForUpdate(options) {
    ...
    const apiKey = this.config?.apiKey;
    if (!apiKey) {
      throw new OTAError('API_KEY_MISSING', 'OTA: apiKey is required. Pass it once in <OTAProvider apiKey="..." /> at your app root.');
    }
    const platform = options?.platform ?? this.config?.platform ?? (Platform.OS === 'ios' ? 'ios' : 'android');
    const bundleId = options?.bundleId ?? this.config?.bundleId ?? getAppId();
    const version_no = options?.version_no ?? options?.appVersion ?? getAppVersion();
    const currentOtaVersion = Number(options?.build_no ?? options?.currentOtaVersion ?? this.config?.currentOtaVersion ?? getOtaVersion());
    ...
    if (rawChannel !== 'development' && rawChannel !== 'production') {
      throw new OTAError('INVALID_CHANNEL', `OTA: Invalid channel "${rawChannel}". Allowed channels are only "development" | "production".`);
    }
    ...
    try {
      ...
      throw new OTAError('NETWORK_ERROR', `OTA: Network request failed when checking for updates — ${err?.message ?? err}`, err);
    } finally {
      clearTimeout(timer);
    }
    if (res.status === 401) {
    ...
    try {
      ...
      throw new OTAError('NETWORK_ERROR', 'OTA: Failed to parse server response as JSON', err);
    }
    ...
    if (rawRelease && (rawRelease.bundleUrl || rawRelease.bundle_url || rawRelease.file_url)) {
      release = {
        ...
        autoRestart: rawRelease.autoRestart !== undefined ? Boolean(rawRelease.autoRestart) : undefined,
    ...
    const isBlackList = Boolean(bundleUrlToCheck ? await OTAStorage.getRejectedUrlInfo(bundleUrlToCheck, nativeAppVersion) : false);
    const result = {
      updateAvailable,
      ...(release ? {
        release
  ...
  async downloadAndApplyUpdate(options) {
    ...
    let url = targetRelease?.bundleUrl ?? targetRelease?.bundle_url ?? targetRelease?.file_url;
    const {
      onProgress,
      onError,
      autoRestart
    ...
    if (!url) {
      const err = new OTAError('DOWNLOAD_FAILED', 'OTA: No release provided and no active release found from checkForUpdate().');
    ...
    if (isBlacklisted) {
      const err = new OTAError('UPDATE_BLACKLISTED', `OTA: Bundle at ${url} was previously rejected and blacklisted for this native build.`);
      onError?.(err.toPayload());
      throw err;
    }
    if (isDownloading) {
      const err = new OTAError('ALREADY_IN_PROGRESS', 'OTA: Another download/update operation is already in progress');
      onError?.(err.toPayload());
      throw err;
    }
    isDownloading = true;
    ...
    try {
      ...
      const zipPath = await OTADownloader.downloadBundle(url, payload => {
        emit(payload);
      }, downloadHeaders);
      emit({
        status: 'downloaded',
      ...
      if (!meta) {
        throw new OTAError('INVALID_META', 'OTA: Missing or invalid meta.json in staged bundle');
      }
      const metaOtaVersion = Number(meta.otaVersion);
      if (!metaOtaVersion || isNaN(metaOtaVersion) || metaOtaVersion <= 0) {
        throw new OTAError('INVALID_META', `OTA: Invalid or missing otaVersion in staged bundle meta.json: "${meta.otaVersion}"`);
      }
      ...
      if (bundleChannel !== currentChannel) {
        throw new OTAError('CHANNEL_MISMATCH', `OTA: Channel mismatch — bundle is built for channel "${bundleChannel}", but device is configured for channel "${currentChannel}"`);
      }
      ...
      const bundleFileName = isAndroid ? OTA_BUNDLE_FILE_NAME : OTA_BUNDLE_FILE_NAME_IOS;
      ...
      await OTAStorage.cleanupStaging();
      if (err instanceof OTAError && (err.code === 'APP_ID_MISMATCH' || err.code === 'APP_VERSION_MISMATCH' || err.code === 'CHANNEL_MISMATCH' || err.code === 'INVALID_META')) {
