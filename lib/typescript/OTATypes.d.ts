...
export interface OTAProgressPayload {
    ...
    downloadedMB: string;
    totalMB: string;
    status: 'idle' | 'checking' | 'downloading' | 'downloaded' | 'installed' | 'upToDate' | 'failed';
}
export type OTAProgressCallback = (payload: OTAProgressPayload) => void;
export type OTAChannel = 'development' | 'production';
export type OTAErrorCode = 'DOWNLOAD_FAILED' | 'EXTRACTION_FAILED' | 'INVALID_META' | 'APP_ID_MISMATCH' | 'APP_VERSION_MISMATCH' | 'CHANNEL_MISMATCH' | 'INVALID_CHANNEL' | 'ALREADY_IN_PROGRESS' | 'STORAGE_ERROR' | 'UPDATE_BLACKLISTED' | 'API_KEY_MISSING' | 'NETWORK_ERROR' | 'UNAUTHORIZED' | 'FORBIDDEN' | 'NO_ACTIVE_RELEASE' | 'UNKNOWN_ERROR';
...
export type OTASyncStatus = 'UP_TO_DATE' | 'UPDATE_INSTALLED' | 'UPDATE_IGNORED' | 'ERROR';
