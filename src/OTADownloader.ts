...
async function performSingleDownload(
  ...
  const { promise } = RNFS.downloadFile({
    ...
    progress: (res) => {
      ...
      const percentage = hasKnownSize
        ? Math.min(Math.round((downloaded / res.contentLength) * 100), 99)
        : -1;
  ...
  if (result.statusCode !== 200) {
    throw new Error(
      `OTA: download failed — HTTP ${result.statusCode} (url: ${url})`
    );
  }
}
...
async function downloadWithRetry(
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (remaining > 0) {
        console.log(
          `OTA: attempt ${attempt + 1} failed — retrying (${remaining} left). Error: ${lastError.message}`
        );
      }
    }
  }
  ...
  await deleteFileIfExists(destPath);
  throw new Error(
    `OTA: download failed after ${MAX_RETRIES + 1} attempts. Last error: ${lastError.message}`
  );
}
